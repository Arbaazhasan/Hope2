...
export const likePost = async (req, res) => {
    ...
    if (!post.likes.includes(userId._id)) {
        await post.updateOne({ $push: { likes: userId._id } });
        ...
        await post.updateOne({ $pull: { likes: userId._id } });
...
export const comment = async (req, res) => {
    const postId = req.params.id
    const {_id, name, profilePicture} = req.user;
...
export const savedPosts = async (req, res) => {
   ...
   try {
    ...
    if (!isPostSaved) {
        const savedPost = await User.updateOne({ _id: userId }, { $push: { savedPosts: postData._id } });
        ...
        const savedPost = await User.updateOne({ _id: userId }, { $pull: { savedPosts: postData._id } });
        if (savedPost) return res.status(200).json({
            success: true,
...
try {
    ...
    await Promise.all(
    savedPostsId.map(async(i)=>{
        const getPost = await Post.findById(i)
        if(getPost)
        {
        savedPostData.push(getPost)
        }
    })
    );
